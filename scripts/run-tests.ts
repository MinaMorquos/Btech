...
// Access `tags` and `fileName` from `RunnerProp`
const { tags } = RunnerProp;

const distCommand = `tsc ./RunnerProp.ts --outDir ./dist`;
const cucumberCommand = `npx cucumber-js src/Features --tags @${tags}`;
const combinedCommand = `${distCommand} && ${cucumberCommand}`;

// Rename 'process' to 'childProcess' to avoid conflicts
