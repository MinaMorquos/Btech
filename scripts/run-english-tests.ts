...
const englishSeqCommand = `npx cucumber-js src/Features/*.feature --tags "@English and @seq" --parallel ${RunnerProp.sequentiallyThreads} --format json:src/Reports/Sequence_Report.json`;
const englishParallelCommand = `npx cucumber-js src/Features/*.feature --tags "@English and not @seq" --parallel ${RunnerProp.parallelThreads} --format json:src/Reports/Parallel_Report.json`;
...
async function runAll(): Promise<void> {
    ...
    await runCommand(englishSeqCommand);
    await runCommand(englishParallelCommand);
